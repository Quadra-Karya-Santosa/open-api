...
@Update()
@Injectable()
export class TelegramSummaryRepository {
  constructor(private readonly seedsTelegramRepo: SeedsTelegramRepository) {}

  private readonly botToken = process.env.TELEGRAM_BOT_SUMMARY_TOKEN;
  ...
  async hearsHi(ctx: Context) {
    ...
    if (exist) {
      ...
      await ctx.reply(
        `You're all set! We'll keep you posted on summary activities in the Seeds app.`,
      );
    }
  }
